...
interface RiskSummaryProps {
  risk?: ChangeRiskResult | null;
  statistics?: ImpactStatistics | null;
    ...
    <div className="rounded-md border border-border-base bg-bg-elevated p-4">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="text-right">
          <SeverityBadge severity={risk?.risk_level ?? 'unknown'} />
          <p className="mt-1 text-2xl font-medium text-text-primary">
            {Number(risk?.risk_score ?? 0).toFixed(0)}
          </p>
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Stat label="Affected nodes" value={String(statistics?.affected_nodes ?? 0)} />
        <Stat label="Propagation paths" value={String(statistics?.propagation_paths ?? 0)} />
        <Stat label="Max depth" value={String(statistics?.max_propagation_depth ?? 0)} />
        <Stat
          label="Confidence"
          value={`${(Number(statistics?.confidence_score ?? 0) * 100).toFixed(0)}%`}
        />
      </div>
