...
class SymbolTableExtractor:
    ...
    def extract(repository_id: str, parsing_result: ProjectParsingResult) -> Dict[str, SymbolMemory]:
        ...
        for file_info in parsing_result.files:
            ...
            # Extract Classes
            for class_symbol in file_info.classes:
                symbol_id = f"{file_info.path}::{class_symbol.name}"
                methods = [m.name for m in file_info.methods] if hasattr(file_info, "methods") else []
            ...
            for func_symbol in all_functions:
                symbol_id = f"{file_info.path}::{func_symbol.name}"
                symbols[symbol_id] = SymbolMemory(
                    metadata=metadata,
            ...
            if hasattr(file_info, "interfaces"):
                for interface_symbol in file_info.interfaces:
                    symbol_id = f"{file_info.path}::{interface_symbol.name}"
                    symbols[symbol_id] = SymbolMemory(
                        metadata=metadata,
                        symbol_name=interface_symbol.name,
            ...
            if hasattr(file_info, "enums"):
                for enum_symbol in file_info.enums:
                    symbol_id = f"{file_info.path}::{enum_symbol.name}"
                    symbols[symbol_id] = SymbolMemory(
                        metadata=metadata,
                        symbol_name=enum_symbol.name,
                        symbol_type="enum",
                        file_path=file_info.path
                    )
                    total_symbols += 1
            ...
            if hasattr(file_info, "variables"):
                for var_symbol in file_info.variables:
                    symbol_id = f"{file_info.path}::{var_symbol.name}"
                    symbols[symbol_id] = SymbolMemory(
                        metadata=metadata,
                        symbol_name=var_symbol.name,
