...
class ContextBuilder:
    ...
    def build(
        ...
        cache_key = CacheKeys.copilot_context(repository_id, f"build:{hash(query) & 0xFFFFFFFF:08x}")
        # Conversation-sensitive — do not return cached full context blindly;
        # only cache repository intelligence fragments.
        ...
        try:
            if repo_fragment and isinstance(repo_fragment, dict):
                ...
                if summary is None:
                    # Build lightweight memory so orchestration has something to compose
                    try:
        ...
        return {
            ...
            "modules_touched": ["Repository Memory", "Advanced RAG"] if rag_context or memory_summary else [],
        }
