...
| ----------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hybrid Search                 | Implemented (Vector + TF-IDF)               | Custom lightweight TF-IDF keyword search avoids external dependencies while capturing exact identifier matches (function names, string literals).                         |
| Configurable Hybrid Ranker    | Implemented                                 | Combines semantic similarity (40%), keyword score (30%), metadata relevance (20%), and repository memory relevance (10%) by default via a configuration object.           |
...
| Deterministic Query Expansion | Implemented                                 | Provides a small, maintainable vocabulary for developer-specific synonyms (e.g., authentication -> auth, jwt) directly in the `QueryAnalyzer`.                            |
| Context Deduplication & Compression| Implemented                            | Deduplicates exact chunks and overlapping ranges. Strips repeated whitespace and non-informational comments, but explicitly preserves structural lines (imports, decorators, class/function signatures). |
